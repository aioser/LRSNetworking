...
#error JMProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "JMProgressHUD.h"
#import "JMIndefiniteAnimatedView.h"
#import "JMProgressAnimatedView.h"
#import "JMRadialGradientLayer.h"
#import <SDWebImage/SDAnimatedImageView.h>

NSString * const JMProgressHUDDidReceiveTouchEventNotification = @"JMProgressHUDDidReceiveTouchEventNotification";
NSString * const JMProgressHUDDidTouchDownInsideNotification = @"JMProgressHUDDidTouchDownInsideNotification";
NSString * const JMProgressHUDWillDisappearNotification = @"JMProgressHUDWillDisappearNotification";
NSString * const JMProgressHUDDidDisappearNotification = @"JMProgressHUDDidDisappearNotification";
NSString * const JMProgressHUDWillAppearNotification = @"JMProgressHUDWillAppearNotification";
NSString * const JMProgressHUDDidAppearNotification = @"JMProgressHUDDidAppearNotification";

NSString * const JMProgressHUDStatusUserInfoKey = @"JMProgressHUDStatusUserInfoKey";

static const CGFloat JMProgressHUDParallaxDepthPoints = 10;
static const CGFloat JMProgressHUDUndefinedProgress = -1;
static const CGFloat JMProgressHUDDefaultAnimationDuration = 0.15;
...
@property (nonatomic, strong) JMProgressAnimatedView *ringView;
@property (nonatomic, strong) JMProgressAnimatedView *backgroundRingView;
...
+ (JMProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static JMProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(JMProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(JMProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(JMProgressHUDAnimationType)type {
...
+ (void)showWithMaskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self sharedView];
    [self showProgress:JMProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status {
    [self showImage:[self sharedView].infoImage status:status animated:false autoDismiss:true imageSize:(CGSizeMake(28, 28))];
}

+ (void)showInfoWithStatus:(NSString*)status maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status {
    [self showImage:[self sharedView].successImage status:status animated:false autoDismiss:true imageSize:(CGSizeMake(28, 28))];
}

+ (void)showSuccessWithStatus:(NSString*)status maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status {
    [self showImage:[self sharedView].errorImage status:status animated:false autoDismiss:true imageSize:(CGSizeMake(28, 28))];
}

+ (void)showErrorWithStatus:(NSString*)status maskType:(JMProgressHUDMaskType)maskType {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status animated:(BOOL)animated autoDismiss:(BOOL)autoDismiss imageSize:(CGSize)imageSize {
    ...
    [[self sharedView] showImage:image status:status duration:displayInterval animated:animated autoDismiss:autoDismiss imageSize:imageSize];
}

+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(JMProgressHUDMaskType)maskType animated:(BOOL)animated autoDismiss:(BOOL)autoDismiss imageSize:(CGSize)imageSize {
    JMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showImage:image status:status animated:animated autoDismiss:autoDismiss imageSize:imageSize];
    [self setDefaultMaskType:existingMaskType];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = JMProgressHUDMaskTypeNone;
        _defaultStyle = JMProgressHUDStyleLight;
        _defaultAnimationType = JMProgressHUDAnimationTypeFlat;
        ...
        _fadeInAnimationDuration = JMProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = JMProgressHUDDefaultAnimationDuration;
        
        _maxSupportedWindowLevel = UIWindowLevelNormal;
        
        // Accessibility support
        self.accessibilityIdentifier = @"JMProgressHUD";
        self.accessibilityLabel = @"JMProgressHUD";
...
#pragma clang diagnostic pop
    ...
    if(self.progress != JMProgressHUDUndefinedProgress) {
...
- (void)updateMask {
    ...
    switch (self.defaultMaskType) {
        case JMProgressHUDMaskTypeCustom:
        case JMProgressHUDMaskTypeBlack:{
            
            self.backgroundLayer = [CALayer layer];
            self.backgroundLayer.frame = self.bounds;
            self.backgroundLayer.backgroundColor = self.defaultMaskType == JMProgressHUDMaskTypeCustom ? self.backgroundLayerColor.CGColor : [UIColor colorWithWhite:0 alpha:0.4].CGColor;
        ...
        case JMProgressHUDMaskTypeGradient:{
            JMRadialGradientLayer *layer = [JMRadialGradientLayer layer];
...
- (void)updateBlurBounds {
    ...
    if(NSClassFromString(@"UIBlurEffect") && self.defaultStyle != JMProgressHUDStyleCustom) {
        ...
        if(self.backgroundColorForStyle != [UIColor clearColor]) {
            // Create blur effect
            UIBlurEffectStyle blurEffectStyle = self.defaultStyle == JMProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    if([self.hudView respondsToSelector:@selector(addMotionEffect:)]) {
        ...
        effectX.minimumRelativeValue = @(-JMProgressHUDParallaxDepthPoints);
        effectX.maximumRelativeValue = @(JMProgressHUDParallaxDepthPoints);
        
        UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
        effectY.minimumRelativeValue = @(-JMProgressHUDParallaxDepthPoints);
        effectY.maximumRelativeValue = @(JMProgressHUDParallaxDepthPoints);
...
#else
        // If JMProgressHUD ist used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo{
    return (self.statusLabel.text ? @{JMProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        ...
        __weak JMProgressHUD *weakSelf = self;
        [UIView animateWithDuration:animationDuration
                         ...
                         animations:^{
            __strong JMProgressHUD *strongSelf = weakSelf;
            if(strongSelf) {
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration animated:(BOOL)animated autoDismiss:(BOOL)autoDismiss imageSize:(CGSize)imageSize {
    __weak JMProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong JMProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            ...
            strongSelf.progress = JMProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != JMProgressHUDMaskTypeNone) {
    ...
    if(self.alpha != 1.0f || self.hudView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(strongSelf) {
                ...
                if(strongSelf.alpha == 1.0f && strongSelf.hudView.alpha == 1.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDDidAppearNotification
                                                                        object:strongSelf
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(JMProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDWillDisappearNotification
                                                                object:nil
                                                              userInfo:[strongSelf notificationUserInfo]];
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.alpha == 0.0f && strongSelf.hudView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:JMProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == JMProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[JMIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[JMIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        JMIndefiniteAnimatedView *indefiniteAnimatedView = (JMIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (JMProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[JMProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (JMProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[JMProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == JMProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == JMProgressHUDStyleDark) {
...
- (BOOL)isClear { // used for iOS 7 and above
    return (self.defaultMaskType == JMProgressHUDMaskTypeClear || self.defaultMaskType == JMProgressHUDMaskTypeNone);
}
...
- (UIView*)hudView {
    ...
    _hudView.backgroundColor = self.defaultStyle == JMProgressHUDStyleCustom ? self.backgroundColor : [UIColor clearColor];
#else
...
- (UIImageView*)imageView {
    if(!_imageView) {
        _imageView = [[SDAnimatedImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 51.0f, 51.0f)];
        _imageView.animationRepeatCount = NSIntegerMax;
        _imageView.shouldCustomLoopCount = true;
    }
...
- (void)setDefaultAnimationType:(JMProgressHUDAnimationType)animationType {
